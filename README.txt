...
Vous pouvez générer des grilles aléatoires avec le script Python suivant :

import random

def generer_grille(nom_fichier, largeur, hauteur, proba_obstacle=0.2):
    with open(nom_fichier, 'w') as fichier:
        fichier.write(f"{largeur} {hauteur}\n")
        for _ in range(hauteur):
            ligne = [
                str(random.choices([0, 1, 2, 3], weights=[70, 20, 5, 5])[0])
