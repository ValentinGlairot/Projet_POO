...
int Grille::compterVoisinsVivants(int x, int y) const {
    int compte = 0; // Compteur de voisins vivants.

    // Parcourt les cellules voisines.
    for (int dx = -1; dx <= 1; ++dx) {
        for (int dy = -1; dy <= 1; ++dy) {
            ...
            if (this->estCelluleValide(nx, ny)) {
                const Cellule& voisin = this->cellules[nx][ny];
                // Si le voisin n'est pas un obstacle et est vivant.
                if (!voisin.obtenirEstObstacle() && voisin.obtenirEtat()) {
                    ++compte; // Incrémente le compteur.
                }
            }
        }
    }

    return compte; // Retourne le nombre de voisins vivants.
}
